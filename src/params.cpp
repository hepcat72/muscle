...
void default_params()
	...
	g_scoreCenter = 0;
	g_scoreGapExtend = 0;
	g_scoreGapOpen2 = MINUS_INFINITY;
	g_scoreGapExtend2 = MINUS_INFINITY;
	g_scoreGapAmbig = 0;
	g_scoreAmbigFactor = 0;
	...
	g_uSmoothWindowLength = 7;
	g_uAnchorSpacing = 32;
	g_uMaxTreeRefineIters = 1;

	g_uRefineWindow = 200;
	g_uWindowFrom = 0;
	g_uWindowTo = 0;
	g_uSaveWindow = uInsane;
	g_uWindowOffset = 0;

	g_uMaxSubFamCount = 5;

	g_uHydrophobicRunLength = 5;
	g_dHydroFactor = (float) 1.2;

	g_uMinDiagLength = 24;	// TODO alpha -- should depend on alphabet?
	g_uMaxDiagBreak = 1;
	g_uDiagMargin = 5;

	g_dSUEFF = (float) 0.1;

	g_bPrecompiledCenter = true;
	g_bNormalizeCounts = false;
	g_bDiags1 = false;
	g_bDiags2 = false;
	g_bAnchors = true;
...
#if	DEBUG
	g_bCatchExceptions = false;
#else
	g_bCatchExceptions = true;
#endif
	...
	g_PPScore = PPSCORE_LE;
	g_ObjScore = OBJSCORE_SPM;

	g_SeqWeight1 = SEQWEIGHT_ClustalW;
	g_SeqWeight2 = SEQWEIGHT_ClustalW;

	g_Distance1 = DISTANCE_Kmer6_6;
	g_Distance2 = DISTANCE_PctIdKimura;

	g_Cluster1 = CLUSTER_UPGMB;
	g_Cluster2 = CLUSTER_UPGMB;
	...
	g_SeqType = SEQTYPE_Auto;

	g_TermGaps = TERMGAPS_Half;

//------------------------------------------------------
// These parameters depending on the chosen prof-prof
// score (g_PPScore), initialized to "Undefined".
	g_dSmoothScoreCeil = fInsane;
	g_dMinBestColScore = fInsane;
	g_dMinSmoothScore = fInsane;
	g_scoreGapOpen = fInsane;
//------------------------------------------------------
...
/*
void SetParams_R(int argc, char *argv[])
	{
	std::string Arg;
	std::string Value;
	...
	for (int iArgIndex = 0; iArgIndex < argc; ){
		...
		// Flags.
		if ( Arg.compare("anchors") == 0 ){
			g_bAnchors = false;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("brenner") == 0 ){
			g_bBrenner = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("cluster") == 0 ){
			g_bClusterOnly = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("dimer") == 0 ){
			g_bDimer = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("clw") == 0 ){
			g_bAln = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("clwstrict") == 0 ){
			g_bClwStrict = true;
			g_bAln = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("core") == 0 ){
			g_bCatchExceptions = false;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("diags") == 0 ){
			g_bDiags = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("diags1") == 0 ){
			g_bDiags1 = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("diags2") == 0 ){
			g_bDiags2 = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("group") == 0 ){
			g_bStable = true;
			++iArgIndex;
			continue;
			}
		...
		if ( Arg.compare("msf") == 0 ){
			g_bMSF = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("phyi") == 0 ){
			g_bPHYI = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("phys") == 0 ){
			g_bPHYS = true;
			++iArgIndex;
			continue;
			}
		...
		if ( Arg.compare("refine") == 0 ){
			g_bRefine = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("refinew") == 0 ){
			g_bRefineW = true;
			++iArgIndex;
			continue;
			}
		if ( Arg.compare("spscore") == 0 ){
			g_bPPScore = true;
			++iArgIndex;
			continue;
			}
		...
		if ( Arg.compare("anchorspacing") == 0 ){
			g_uAnchorSpacing = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("center") == 0 ){
			g_scoreCenter = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("cluster1") == 0 ){
			if( Value.compare("upgma") == 0 ){
				g_Cluster2 = CLUSTER_UPGMA;
			}else if( Value.compare("neighborjoining") == 0 ){
				g_Cluster2 = CLUSTER_NeighborJoining;
				}
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("cluster2") == 0 ){
			if( Value.compare("upgma") == 0 ){
				g_Cluster2 = CLUSTER_UPGMA;
			}else if( Value.compare("neighborjoining") == 0 ){
				g_Cluster2 = CLUSTER_NeighborJoining;
		...
		if ( Arg.compare("clwout") == 0 ){
			g_pstrClwOutFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("diagbreak") == 0 ){
			g_uMaxDiagBreak = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("diaglength") == 0 ){
			g_uMinDiagLength = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("diagmargin") == 0 ){
			g_uDiagMargin = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("distance1") == 0 ){
			if( Value.compare("kmer6_6") == 0 ){
				g_Distance1 = DISTANCE_Kmer6_6;
			}else if( Value.compare("kmer20_3") == 0 ){
				g_Distance1 = DISTANCE_Kmer20_3;
			}else if( Value.compare("kmer20_4") == 0 ){
				g_Distance1 = DISTANCE_Kmer20_4;
			}else if( Value.compare("kbit20_3") == 0 ){
				g_Distance1 = DISTANCE_Kbit20_3;
			}else if( Value.compare("kmer4_6") == 0 ){
				g_Distance1 = DISTANCE_Kmer4_6;
				}
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("distance2") == 0 ){
			if( Value.compare("pctid_kimura") == 0 ){
				g_Distance1 = DISTANCE_PctIdKimura;
			}else if( Value.compare("pctid_log") == 0 ){
				g_Distance1 = DISTANCE_PctIdLog;
				}
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("fastaout") == 0 ){
			g_pstrFASTAOutFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("gapopen") == 0 ){
			g_scoreGapOpen = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("hydro") == 0 ){
			g_uHydrophobicRunLength = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("hydrofactor") == 0 ){
			g_dHydroFactor = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("in") == 0 ){
			g_pstrInFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("in1") == 0 ){
			g_pstrFileName1 = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("in2") == 0 ){
			g_pstrFileName2 = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("matrix") == 0 ){
			g_pstrMatrixFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("maxiters") == 0 ){
			g_uMaxIters = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("maxtrees") == 0 ){
			g_uMaxTreeRefineIters = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("minbestcolscore") == 0 ){
			g_dMinBestColScore = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("minsmoothscore") == 0 ){
			g_dMinSmoothScore = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("msaout") == 0 ){
			g_pstrMSFOutFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("objscore") == 0 ){
			if( Value.compare("sp") == 0 ){
				g_ObjScore = OBJSCORE_SP;
			}else if( Value.compare("ps") == 0 ){
				g_ObjScore = OBJSCORE_PS;
			}else if( Value.compare("dp") == 0 ){
				g_ObjScore = OBJSCORE_DP;
			}else if( Value.compare("xp") == 0 ){
				g_ObjScore = OBJSCORE_XP;
			}else if( Value.compare("xp") == 0 ){
				g_ObjScore = OBJSCORE_XP;
			}else if( Value.compare("spf") == 0 ){
				g_ObjScore = OBJSCORE_SPF;
				}
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("out") == 0 ){
			g_pstrOutFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("phyiout") == 0 ){
			g_pstrPHYIOutFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("physout") == 0 ){
			g_pstrPHYSOutFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("refinewindow") == 0 ){
			g_uRefineWindow = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("scorefile") == 0 ){
			g_pstrScoreFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("smoothscoreceil") == 0 ){
			g_dSmoothScoreCeil = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("smoothwindow") == 0 ){
			g_uSmoothWindowLength = atoi(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("spscore") == 0 ){
			g_pstrSPFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("SUEFF") == 0 ){
			g_dSUEFF = atof(Value.c_str());
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("usetree") == 0 ){
			g_pstrUseTreeFileName = Value.c_str();
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("weight1") == 0){
			if( Value.compare("none") == 0 ){
				g_SeqWeight1 = SEQWEIGHT_None;
			}else if( Value.compare("henikoff") == 0 ){
				g_SeqWeight1 = SEQWEIGHT_Henikoff;
			}else if( Value.compare("henikoffpb") == 0 ){
				g_SeqWeight1 = SEQWEIGHT_HenikoffPB;
			}else if( Value.compare("gsc") == 0 ){
				g_SeqWeight1 = SEQWEIGHT_GSC;
			}else if( Value.compare("threeway") == 0 ){
				g_SeqWeight1 = SEQWEIGHT_ThreeWay;
				}
			iArgIndex += 2;
			continue;
			}
		if ( Arg.compare("weight2") == 0){
			if( Value.compare("none") == 0 ){
				g_SeqWeight2 = SEQWEIGHT_None;
			}else if( Value.compare("henikoff") == 0 ){
				g_SeqWeight2 = SEQWEIGHT_Henikoff;
			}else if( Value.compare("henikoffpb") == 0 ){
				g_SeqWeight2 = SEQWEIGHT_HenikoffPB;
			}else if( Value.compare("gsc") == 0 ){
				g_SeqWeight2 = SEQWEIGHT_GSC;
			}else if( Value.compare("threeway") == 0 ){
				g_SeqWeight2 = SEQWEIGHT_ThreeWay;
				}
			iArgIndex += 2;
			continue;
			}
