...
static bool TestSetValueOpt(const char *Arg, const char *Value)
	...
	for (int i = 0; i < ValueOptCount; ++i)
		if (!stricmp(Arg, ValueOpts[i].m_pstrName))
			...
			if (0 == Value)
				{
				Rprintf("Option -%s must have value\n", Arg);
				return 0;
				}
...
void ProcessArgVect(int argc, char *argv[])
	...
	for (int iArgIndex = 0; iArgIndex < argc; )
		...
		//if (Arg[0] != '-')
		//	{
		//	Rprintf("Command-line option \"%s\" must start with '-'\n", Arg);
		//	return;
		//	}
...
// Blank parameters for R.

void blank_params()
	{
	int i;
	// Values.
	for(i = 0; i < ValueOptCount; ++i){
		ValueOpts[i].m_pstrValue = 0;
		}
	// Flags.
	for(i = 0; i < FlagOptCount; ++i){
		FlagOpts[i].m_bSet = false;
		}
	}
